...
#Data located here:
alldata_unmasked <- read.csv("DataOut_11282018.csv")
...
myplot <- ggplot(alldata, aes(x=TIMEPOINT,y=(NORMALIZED_GFP_MEDIAN), colour=SAMPLE, label = SAMPLE)) +
...
myplot %+% subset(alldata, VESSEL %in% c('1'))
#myplot %+% subset(alldata, TIMEPOINT %in% c("ON"))
...
myplot %+% subset(alldata, VESSEL %in% NA
myplot %+% subset(alldata, TIMEPOINT %in% c("ON"))
```
