...
flowData <- read.flowSet(path = paste(path.data,list.folders[2],"/", sep=""), pattern=".fcs", alter.names = TRUE)

sample.sheet <- read.csv(paste(path.data,"Samplesheet_",list.folders[2],".csv", sep=""))
...
pData(flowData)$Run <- list.folders[2]
...
pfsc <-ggplot(filter.data)+
      ...
      theme(legend.position="none", strip.text = element_text(size = 4))+
      labs(y = "Counts") +
      scale_x_log10()+
      facet_wrap('SAMPLE')
