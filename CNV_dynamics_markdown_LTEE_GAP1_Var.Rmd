...
data <- read_csv(paste(list.folders[1],"_prop_",date,".csv", sep=""))
data <- data %>% add_column(TIMEPOINT=timepoint[1]) %>% select(-SAMPLE)


for(i in 2:length(list.folders)){
  data.temp <- read_csv(paste(list.folders[i],"_prop_",date,".csv", sep=""))
...
i<-1
name <- list.folders[i]
load(paste(path.data,name,"_df_",date,".Rdata",sep=""))
data2 <- cbind(filtered.data, TIMEPOINT=timepoint[i])
for(i in 2:length(list.folders)) {
  name <- list.folders[i]
  load(paste(path.data,name,"_df_",date,".Rdata",sep=""))
...
filtered.data$gens<-factor(data2$gens,levels=unique(data2$gens))

ggplot(filtered.data, aes(x = FL1.A/FSC.A, y = SAMPLE, fill = ..x.., height=..density..)) +
