...
files<-list.files(paste(path.data, list.folders[tp],"/", sep=""),full.names=TRUE)
...
pData(flowData)$Run <- list.folders[tp]

if(all == "YES") {
  for(i in 2:length(list.folders)) {
    
    files<-list.files(paste(path.data, list.folders[i],"/", sep=""),full.names=TRUE)
    flowData.temp <- read.ncdfFlowSet(files=files, pattern=".fcs", alter.names = TRUE)
...
write.table(all.data, file="DataOut_12062018.csv", row.names=TRUE, quote=F, sep=",")
```
