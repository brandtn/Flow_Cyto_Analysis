...
filter.data$TIMEPOINT <- factor(filter.data$TIMEPOINT, levels(factor(filter.data$TIMEPOINT))[c(11,1,8:10,2:7)]) # fixes timepoint order
...
plot.data <- filter.data %>% filter(VESSEL == filt) %>% arrange(TIMEPOINT)
