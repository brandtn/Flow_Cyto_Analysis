...
sample.param <- data.frame(SAMPLE=NA, WELL=NA, STRAIN=NA, GENOTYPE=NA, PLOIDY=NA, MEDIA=NA, EXPERIMENT=NA, POPULATION = NA, TIMEPOINT= NA)
...
save.flowset <- "No"
#filename
folder.flowset <- paste(name,"flowdata",Sys.Date(),sep="")
...
load(file = paste(name,"_gates_",date,".Rdata",sep=""))
...
for(i in 1:length(flowData)){
  ...
  filtered.data <- rbind(filtered.data, cbind(SAMPLE=sample,WELL=well,STRAIN=strain,GENOTYPE=genotype,PLOIDY=ploidy,MEDIA=media,EXPERIMENT=exp, POPULATION = pop, TIMEPOINT= time,
...
if(save.prop == "Yes"){write.table(all.data, file=paste(file.prop,".csv",sep=""), row.names=TRUE, append=(add.data == "Yes"), quote=F, sep=",")}
```
...
else {if(save.df == "Yes"){save(filtered.data, file=paste(file.df,".Rdata",sep=""))}}

if(save.stats== "Yes"){write.table(stats.data, file= file.stats, row.names=TRUE, append=(add.data == "Yes"), quote=F, sep=",")}
